...
# Optional model routing (both default to OPENAI_MODEL):
OPENAI_EVAL_MODEL=
OPENAI_FAST_MODEL=
...
| `OPENAI_MODEL` | No | Primary model used by the OpenAI client. |
| `OPENAI_EVAL_MODEL` | No | Stronger model reserved for grounded candidate assessment. Defaults to `OPENAI_MODEL`. |
| `OPENAI_FAST_MODEL` | No | Cheaper/faster model for high-volume calls (signal generation, summaries). Defaults to `OPENAI_MODEL`. |
| `OPENAI_REASONING_EFFORT` | No | Optional reasoning effort (`low`/`medium`/`high`) for reasoning-capable models. Left blank by default. |
| `OPENAI_MAX_OUTPUT_TOKENS` | No | Optional per-call output-token cap (`0` = API default). |
...
Schema changes are managed with **Alembic** (`backend/alembic/`). The baseline
revision captures the full current schema and is idempotent, so it is safe to
...
# Existing/deployed database (e.g. the current Neon instance): mark the baseline
# as applied without touching the already-created schema.
alembic stamp head
```

Add future schema changes as new autogenerated revisions:
...
The Alembic environment reads `DATABASE_URL` from your `.env`, so no database
credentials are stored in `alembic.ini`. The legacy `create_tables.py` and the
runtime `schema_guard` remain for quick local bootstrapping, but Alembic is the
source of truth for controlled schema evolution going forward.

---

## Learning Loop (Signal Weights)

Recruiter feedback adjusts deterministic **signal weights** (`SignalWeight`) and
outcome **task weights**. These learned weights are loaded via
`load_signal_weights()` and applied during candidate screening, deep evaluation,
and single-outcome evaluation, so feedback measurably shifts future scores
rather than only being recorded for audit. Admins can inspect the current
weights and the full change history from the admin panel.
